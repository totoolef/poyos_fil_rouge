<template>
  <v-card>
    <v-card-title class="headline pa-4">
      <h2 class="mb-0">Inscription Annonceur</h2>
    </v-card-title>
...
const soumettreInscription = async () => {
  ...
  try {
    const reponse = await axios.post('http://localhost:8080/authentification/inscription_annonceur.php', {
    ...
    if (reponse.data.success) {
      ...
      fermerDialog();
      router.push('/dashboard-annonceur/annonces-postees')
    }
  } catch (erreur) {
